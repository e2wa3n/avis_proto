...
def load_node_registry(path="conf/node_registry.json"):
    try:
        with open(path) as f:
            return json.load(f)
    except Exception as e:
...
def main():
    ...
    while True:
        ...
        print(f"[DEBUG] raw packet from {addr}: {data!r}")

        # short‑circuit anything too small to even have a Semtech header
        if len(data) < 4:
            continue
        ...
        if pkt == 0x00:
            ...
            print(f"[DEBUG] unknown packet type 0x{pkt:02x}")
